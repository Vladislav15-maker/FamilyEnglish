...
export async function GET(
  ...
  try {
    ...
    const memberCheck = await sql`SELECT 1 FROM chat_group_members WHERE group_id = ${groupId} AND user_id = ${user.id};`;
    if (memberCheck.rowCount === 0) {
      return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
    }

    const [groupRes, membersRes, messagesRes] = await Promise.all([
      sql`SELECT id, name, created_at FROM chat_groups WHERE id = ${groupId};`,
